...
import { Anomaly, AnalysisStatus, DashboardData, Language, Theme, ViewState, Session, DashboardLayout, LayoutItem, FileHistoryEntry, WidgetConfig } from './types';
...
export default function App() {
  ...
  const handleSettingsUpdate = useCallback((newLang: Language, newTheme: Theme, privateMode: boolean) => {
    if (session) {
      const newSettings = {
        language: newLang,
        theme: newTheme,
  ...
  const handleLayoutChange = useCallback((newLayout: Layout[]) => {
    setCurrentLayout(prev => ({
      ...prev,
      layout: [...newLayout] as unknown as LayoutItem[]
    }));
  }, []);
  ...
  // Dashboard customization views are now defined outside or as memoized components to prevent remounting
  // on every state change (like opening notifications).
  ...
  const renderContent = () => {
    // 1. Always available global views
    ...
    // 2. Status-based rendering for data-dependent views
    ...
    switch (currentView) {
      ...
      default:
        return null; // The cases above cover all ViewState types
    }
  };
          ...
          <header className="flex justify-between items-center mb-4 flex-shrink-0 h-12">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mt-0.5">
                {fileName ? `${t.status.file}: ${fileName}` : 'NordBund Intelligence'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 relative">
              <button
                ...
                className={`p-2.5 bg-white dark:bg-slate-800 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 relative transition-colors border ${isNotificationsOpen ? 'border-blue-500 ring-2 ring-blue-100 dark:ring-blue-900/30' : 'border-slate-100 dark:border-slate-800'}`}
              >
                <Bell size={18} className={isNotificationsOpen ? 'text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'} />
...
// COMPONENT VIEWS (Moved outside to prevent remounting)
...
interface ViewOverviewProps {
  ...
  handleSaveLayout: (layout: DashboardLayout) => void;
  handleLoadLayout: (id: string) => void;
  handleDeleteLayout: (id: string) => void;
  handleUpdateWidgets: (widgets: WidgetConfig[]) => void;
}

const ViewOverview = React.memo<ViewOverviewProps>((props) => {
  const {
    data, theme, currentLayout, isEditMode, isConfigOpen,
    toggleEditMode, setIsConfigOpen, handleLayoutChange,
    lang, t, savedLayouts, handleSaveLayout, handleLoadLayout,
    handleDeleteLayout, handleUpdateWidgets
  } = props;
    ...
    <div className="h-full flex flex-col gap-2 animate-in fade-in duration-500 overflow-hidden relative">
      <div className="flex items-center justify-end gap-2 flex-shrink-0 px-2">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg font-semibold text-xs transition-colors flex items-center gap-2 ${isEditMode
...
const ViewStatistics = React.memo<BaseViewProps>(({ data, theme, lang, t }) => (
...
const ViewClaims = React.memo<BaseViewProps>(({ data, theme, lang, t }) => (
...
const ViewCustomers = React.memo<BaseViewProps>(({ data, theme, lang, t }) => (
...
const ViewAnalytics = React.memo<AnalyticsViewProps>(({ data, lang, t }) => (
...
interface ViewSettingsProps {
  ...
  handleSettingsUpdate: (lang: Language, theme: Theme, privateMode: boolean) => void;
}

const ViewSettings = React.memo<ViewSettingsProps>(({ lang, theme, t, session, handleSettingsUpdate }) => {
  const handleLangChange = (newLang: Language) => {
    handleSettingsUpdate(newLang, theme, session?.settings.privateMode || false);
  };

  const handleThemeChange = (newTheme: Theme) => {
    handleSettingsUpdate(lang, newTheme, session?.settings.privateMode || false);
  };
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
          ...
          <button
            ...
            className={`w-full py-2 px-4 rounded-lg font-medium transition-colors ${session?.settings.privateMode
...
interface ViewHistoryProps {
  ...
  handleLoadFromHistory: (entry: FileHistoryEntry) => void;
...
const ViewHistory = React.memo<ViewHistoryProps>(({ history, handleLoadFromHistory, removeFromHistory, clearHistory, lang, t }) => (
